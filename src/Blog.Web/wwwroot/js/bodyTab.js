...
layui.define(["element", "jquery"], function (exports) {
    ...
    Tab.prototype.navBar = function (strData) {
        ...
        if (typeof (strData) == "string") {
            var json = JSON.parse(strData); //部分用户解析出来的是字符串，转换一下
            data = json.menus;
            encode = json.code;
        } else {
            data = strData.menus;
            ...
            if (data[i].children != undefined && data[i].children.length > 0) {
                ...
                for (var j = 0; j < data[i].children.length; j++) {
                    if (data[i].children[j].target == "_blank") {
                        ulHtml += '<dd><a data-code="' + encode + '" data-id="' + data[i].children[j].id + '" data-url="' + data[i].children[j].href + '" target="' + data[i].children[j].target + '">';
                    } else {
                        ulHtml += '<dd><a data-code="' + encode + '" data-id="' + data[i].children[j].id + '" data-url="' + data[i].children[j].href + '">';
                    }
                ...
                if (data[i].target == "_blank") {
                    ulHtml += '<a data-code="' + encode + '" data-id="' + data[i].id + '" data-url="' + data[i].href + '" target="' + data[i].target + '">';
                } else {
                    ulHtml += '<a data-code="' + encode + '" data-id="' + data[i].id + '" data-url="' + data[i].href + '">';
                }
                if (data[i].icon != undefined && data[i].icon != '') {
                    ulHtml += '<i class="' + data[i].icon + '" data-icon="' + data[i].icon + '"></i>';
                }
    ...
    Tab.prototype.render = function () {
        //显示左侧菜单
        var _this = this;
        $(".navBar ul").html('<li class="layui-nav-item layui-this"><a data-url="/MainManage/Home/Default"><i class="layui-icon" data-icon=""></i><cite>后台首页</cite></a></li>').append(_this.navBar(dataStr)).height($(window).height() - 210);
    ...
    /**获取当前iframe对应lay-id */
    Tab.prototype.getSelfLayId = function () {
        var $self = $(".layui-tab-title.top_tab li[class='layui-this']");
        if ($self.length > 0) {
            return $self.attr("lay-id");
        }
        return "";
    }
    ...
    Tab.prototype.tabAdd = function (_this) {
        ...
        if (_this.attr("target") == "_blank") {
            ...
            var title = '';
            if (_this.find("i").attr("data-icon") != undefined) {
            ...
            if (that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0) {
                ...
                //当前窗口内容
                var curmenu = {
                    "icon": _this.find("i").attr("data-icon") != undefined ? _this.find("i").attr("data-icon") : _this.find("i").attr("data-icon"),
                    "title": _this.find("cite").text(),
                    "href": _this.attr("data-url"),
                    "layId": layId, //new Date().getTime()
                    "iframeId": tabIdIndex
                }
                menu.push(curmenu);
    ...
    //重新选中左侧菜单
    function activeMenu() {
        var lid = $("#top_tabs>.layui-this").attr("lay-id");
        if (lid) {
            var $curr = $("#navBar a[data-id=" + lid + "]");
            if ($curr.length > 0) {
                $curr.parent().addClass("layui-this").siblings().removeClass("layui-this");
                $curr.parent().siblings().find("dd").removeClass("layui-this");
            } else {
                $("#navBar .layui-this").removeClass("layui-this");
            }
        } else {
            $("#navBar li:eq(0)").addClass("layui-this").siblings().removeClass("layui-this");
            $("#navBar li:eq(0)").siblings().find("dd").removeClass("layui-this");
        }
    }
