...
layui.use(['bodyTab', 'tools'], function () {
    var form = layui.form,
        ...
        tools.moduleOpen({
            ...
            callBack: function (index, layero) {
                ...
                iframeWindow.layui.form.on("submit(save)", function (data) {
                    if (data.field.Password.length < 6 || data.field.OldPassword.length < 6) {
    ...
    function lockPage() {
        ...
        layer.open({
            ...
            content: '<div class="admin-header-lock" id="lock-box">' +
                '<div class="admin-header-lock-img"><img src="' + img + '" class="userAvatar"/></div>' +
    ...
    $("body").on("click", "#unlock", function () {
        if ($(this).siblings(".admin-header-lock-input").val() == '') {
            ...
            $.post("/main/home/lock", { pwd: pwd }, function (result) {
                layer.close(loading);
                if (result.Status == 0) {
    ...
    function initMenu() {
        ...
        tools.get({
            url: "/SystemManage/Module/GetMenuAndButton", success: function (data) {
                topMenu = data.topMenus;
                childMenu = data.childMenus;
                toolButton = data.toolButtons;
                rowButton = data.rowButtons;
                layui.each(topMenu, function (index) {
                    if (index == 0) {
                        defaultMneuId = this.id;
                    }
                    mobile += '<dd ' + (index == 0 ? 'class="layui-this"' : '') + ' data-menu="' + this.id + '">';
                    mobile += '<a href="javascript:;">';
                    mobile += '<i class="' + this.icon + '" data-icon="' + this.icon + '"></i>';
                    mobile += '<cite>' + this.title + '</cite></a></dd>';

                    pc += '<li class="layui-nav-item ' + (index == 0 ? 'layui-this' : '') + '" data-menu="' + this.id + '" pc>';
                    pc += ' <a href="javascript:;"><i class="' + this.icon + '" data-icon="' + this.icon + '"></i><cite>' + this.title + '</cite></a>';
                    pc += '</li>';
                });
                $("#pcMenu").html(pc);
    ...
    //页面加载时判断左侧菜单是否显示
    //通过顶部菜单获取左侧菜单
    $(".topLevelMenus,.mobileTopLevelMenus").on("click", "li,dd", function () {
