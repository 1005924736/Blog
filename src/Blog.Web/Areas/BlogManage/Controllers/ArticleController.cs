...
namespace Blog.Web.Areas.BlogManage.Controllers
{
    ...
    public class ArticleController : BaseControler
    {
        ...
        {
            return Json(_articleInfoService.QueryableByPage(query), "yyyy-MM-dd HH:mm:ss");
        }
        ...
        {
            return Json(_articleInfoService.Save(article), "yyyy-MM-dd HH:mm:ss");
        }
        ...
        {
            var article = _articleInfoService.FindEntity(c => c.ArticleId == key);
            article.Categories = _articleCategoryService.Queryable(c => c.ArticleId == key).Select(k => k.CategoryId).ToList();
            article.Tags = _articleTagsService.Queryable(c => c.ArticleId == key).Select(k => k.TagsId).ToList();
        ...
        {
            return Json(_articleInfoService.Update(a => new ArticleInfo { IsTop = status }, c => c.ArticleId == id));
        }
        ...
        {
            return Json(_articleInfoService.Update(a => new ArticleInfo { Visible = status }, c => c.ArticleId == id));
        }
